...
'''
    CONFIG:
        ...
        - print data to file "random_benchmark_"+method+"_data.txt"
...
def plot_one(method):
    ...
    if print_data_table:
        ...
        f = open("random_benchmark_"+method+"_data.txt", "w")
        i = 0
    ...
    for i in range(len(evolution_dots)):
    ...
    plt.savefig('random_benchmark_'+methods_short[method]+sufix+'.png')
    plt.savefig('random_benchmark_'+methods_short[method]+sufix+'.pdf')
...
def plot_histogram():
    # number of better datasets than Minimum Randomized Search Benchmark
    ...
    plt.xticks(range(0, len(methods_list)), method_list, rotation='vertical')        
    
    plt.subplots_adjust(bottom=0.40)
    
    plt.savefig('random_min_evolution_annealing.png')
    plt.savefig('random_min_evolution_annealing.pdf')
