...
'''
    CONFIG:
        ...
        - plot full labels with datasets names or just datasets ids
            figure_with_labels   True/False    

        - plot values with default settings, mark by square 
            plot_defaults         True/False
            
'''

figure_with_labels = True
plot_defaults = True
...
try:
    ...
    print("Datasets order could not be read from the file labels_indexes.txt, run plot_errors_ranges_all_in_one.py first")
    exit()
...
def plot_one(method):
    ...
    if not figure_with_labels:
        sufix += "_nolabels"
         
    
    plt.savefig(methods_short[method]+sufix+'.png')
    plt.savefig(methods_short[method]+sufix+'.pdf')
...
def get_data_from_db(method, dataset):
    ...
    if mydb:
        ...
        val = (dataset, methods_short[method])
        
        sql = "SELECT accuracy FROM results WHERE dataset=%s AND method=%s AND accuracy IS NOT NULL AND preprocessings='[]'"
...
for method in method_list:
    try:
        f = open(path_default_folder+methods_short[method]+".def.res")
        for line in f.readlines():            
            openml_id, accuracy, std, params = line.split(" ", maxsplit=3)
            default[(method, int(openml_id))] = 1-float(accuracy) 
        f.close()
    except:
        print(path_default_folder+methods_short[method]+".def.res could not be processed")
